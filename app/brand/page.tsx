...
export default function BrandPage() {
  return <main className="policy-page brand-page">
    ...
    <section><h2>Version 1.0 Brand Variations</h2><p>These pilot marks preserve the Esther Funds Foundation royal purple, lavender, warm cream, white, Dove-centered identity, university typography, official motto, establishment year, and campaign line.</p><figure className="official-wordmark"><img src="/effu-official-university-wordmark.jpg" alt="EFFU — Esther Funds Foundation University" /><figcaption>Official EFF University primary and horizontal wordmark supplied by Esther Funds Foundation.</figcaption></figure><div className="brand-grid">{marks.map(([label, style, title, subtitle]) => <article key={label}><small>{label}</small><div className={`brand-mark ${style}`}><img src="/eff-university-dove-crest.png" alt="" /><span><b>{title}</b><i>{subtitle}</i></span></div></article>)}</div></section>

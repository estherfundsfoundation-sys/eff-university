...
export default function BrandPage() {
  return <main className="policy-page brand-page">
    ...
    <section><h2>Version 1.0 Brand Variations</h2><p>These pilot marks preserve the Esther Funds Foundation royal purple, lavender, warm cream, white, dove-centered identity, university typography, official motto, establishment year, and campaign line.</p><div className="brand-grid">{marks.map(([label, style, title, subtitle]) => <article key={label}><small>{label}</small><div className={`brand-mark ${style}`}><img src="/eff-university-dove-crest.png" alt="" /><span><b>{title}</b><i>{subtitle}</i></span></div></article>)}</div></section>

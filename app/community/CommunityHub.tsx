...
export default function CommunityHub({ accountName, signOutPath }: { accountName: string; signOutPath: string }) {
  ...
  return <main className="community-page">
    ...
    {tab === "profile" && <section className="profile-editor"><div><p className="eyebrow">MY EFFU ACCOUNT</p><h2>Your returning-student account</h2><p>This account is saved securely to your sign-in so you can return from any supported device. Your profile, interests, community posts, and EFFU connections remain available when you log back in.</p><div className="account-security"><b>YOUR PRIVACY</b><span>Your account email identifies your private record but is never displayed publicly. Choose a display name and never post passwords, addresses, school schedules, financial records, or private contact information.</span></div></div><form onSubmit={async (event) => { event.preventDefault(); if (await action({ type: "profile", ...profile })) { setNotice("Account profile saved. You can return and sign in anytime."); setTab("community"); } }}>
      ...
      <button type="submit">SAVE MY RETURNING-STUDENT ACCOUNT →</button>
    </form></section>}
